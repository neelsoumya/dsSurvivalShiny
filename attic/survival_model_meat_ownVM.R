##############################################################################
# Meta-analysis of survival models in DataSHIELD for the meat InterAct project
#
# Additional code in:
...
#
# Usage:
#     R --no-save < survival_model_meat_ownVM.R > output_meat_surv.txt
#
##############################################################################
...
# NUTS_SEEDS is the exposure and CASE_OBJ is the event. 
...
#	NUTSSEEDS + ALCOHOL + PA + SMOKING
#	 + EDUCATION + E_INTAKE
#############################################
#dsBaseClient::ds.Surv(time='STARTTIME', time2='ENDTIME', event = 'EVENT', objectname='surv_object2', type='counting')
coxph_model_full <- dsBaseClient::ds.coxph.SLMA(formula = 'surv_object ~ AGEBASE + GENDER + NUTSSEEDS + ALCOHOL + PA + SMOKING + EDUCATION + E_INTAKE')

dsBaseClient::ds.coxph.SLMA(formula = 'surv_object ~ AGEBASE + GENDER + NUTSSEEDS + ALCOHOL + PA + SMOKING + EDUCATION + E_INTAKE',
                            combine_with_metafor = TRUE)

# Prentice weighted Cox proportional hazards model
dsBaseClient::ds.coxph.SLMA(formula = 'surv_object_prentice ~ AGEBASE + GENDER + NUTSSEEDS + ALCOHOL + PA + SMOKING + EDUCATION + E_INTAKE',
                            combine_with_metafor = TRUE)
...
dsBaseClient::ds.coxphSLMAassign(formula = 'surv_object ~ AGEBASE + GENDER + NUTSSEEDS + ALCOHOL + PA + SMOKING + EDUCATION + E_INTAKE', 
...
input_logHR = c(coxph_model_full$study1$coefficients[2,2], 
                coxph_model_full$study2$coefficients[2,2]#, 
                #coxph_model_full$study3$coefficients[2,2]
                )
input_se    = c(coxph_model_full$study1$coefficients[2,3], 
                coxph_model_full$study2$coefficients[2,3]#, 
                #coxph_model_full$study3$coefficients[2,3]
                )
