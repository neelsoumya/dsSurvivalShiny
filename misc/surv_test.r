...
survfit_cox$lower <- abs(rnorm(n = length(survfit_cox$lower), mean = survfit_cox$lower, sd = 0.1))
survfit_cox$upper <- abs(rnorm(n = length(survfit_cox$upper), mean = survfit_cox$upper, sd = 0.1))
plot(survfit_cox)
plot(survfit_cox, fun="cloglog")

#########################################
# add random noise to surv object
#########################################
# for (i_temp_counter_loop in c(1:length(STARTTIME)) )
# {
#    i_temp_counter_loop  
# }  

STARTTIME_noise <- abs(rnorm(n = length(STARTTIME), mean = STARTTIME, sd = 0.0001))
ENDTIME_noise   <- abs(rnorm(n = length(ENDTIME), mean = ENDTIME, sd = 0.0001))

# TODO: EVENT not modified amd ENDTIME not modified

s_noise <- survival::Surv(time = STARTTIME_noise, 
                          #time2 = ENDTIME_noise,
                          event = EVENT)
summary(s_noise)

coxph_object_noise <- survival::coxph(formula = s_noise ~ AGE, data=file)
summary(coxph_object_noise)

# Kaplan-Meier plot
survfit_km_noise <- survival::survfit(formula = s_noise ~ 1 )
plot(survfit_km_noise)
